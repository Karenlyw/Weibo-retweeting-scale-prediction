...
file= open(r'/Users/xuyuemei/xuyuemei/Work/1Research/文章/转发规模/Code/数据/gbdt_data.csv',encoding='gbk')
...
for i in range(0,100):
    ...
    cnn_score=cnn_score+(model.feature_importances_)[0]
    cos_score=cos_score+(model.feature_importances_)[1]
    fans_score=fans_score+(model.feature_importances_)[2]
    daily_score=daily_score+(model.feature_importances_)[3]
    popular_score=popular_score+(model.feature_importances_)[4]
    time_score=time_score+(model.feature_importances_)[5]
...
for i in gbdt_test_f1:
    f=f+i
print(f/100)
print("score of cnn,cos,fans,daily,popular,time")
#'cnn打分','cos打分','发布者粉丝数','发布者日均发博数','发布者热转率','发布时间段'
print(cnn_score/100)
print(cos_score/100)
