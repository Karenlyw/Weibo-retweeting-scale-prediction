...
def test():
    ...
    t1=time.time()
    x_test,y_test=process_file3(config.test_filename,word_to_id,cat_to_id,config.seq_length)
    ...
    for i in range(num_batch):
        ...
        session.run(model.prob,feed_dict=feed_dict)
        for p in session.run(model.prob,feed_dict=feed_dict):
            y_prob.append(p[0])
    ...
    with open(r'C:\Users\Administrator\Desktop\score.txt','w',encoding='utf-8') as f:
