...
class TextConfig():
    ...
    seq_length=140      #max length of sentence
    ...
    train_filename='./data/十折交叉1/train1.txt'  #train data
    test_filename='./data/test.txt'    #test data
    val_filename='./data/十折交叉1/val1.txt'      #validation data
...
class TextCNN(object):
    ...
    def cnn(self):
        ...
        with tf.name_scope('cnn'):
            ...
            outputs= tf.reduce_max(conv, reduction_indices=[1], name='gmp')#全局最大池化

        with tf.name_scope('fc'):
            self.fc=tf.layers.dense(outputs,self.config.hidden_dim,name='fc1')
            fc = tf.nn.dropout(self.fc, self.keep_prob)
            fc=tf.nn.relu(fc)
